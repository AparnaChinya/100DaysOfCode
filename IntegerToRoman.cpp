...
public:
    string intToRoman(int num) {
        ...
        vector<string> single = {"","I","II","III","IV","V","VI","VII","VIII","IX"};

        string tho = thousand[num / 1000];
        string hun = hundred[(num % 1000) / 100];
        string ten = tens[(num % 100)/10];
        string sin = single[(num % 10)];

        return tho + hun + ten + sin;
    }
};
